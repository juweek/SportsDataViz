...
import { sortList, sortDropdownHandler } from './sort.js';
...
function showClicked(currentlySelectedCountries, data) {
    console.log(currentlySelectedCountries)
...
------------------------
*/
function createSearchResults(data, searchValue, playersList, searchBarResultsDiv) {
  //loop through the data and create a new array that only has the countries with the searchValue in them
  const playersFromSearch = playersList.filter(result => result.includes(searchValue.toLowerCase()));
  let html = '';
  ...
  if (searchValue === '') {
    ...
    playersFromSearch.forEach(result => {
